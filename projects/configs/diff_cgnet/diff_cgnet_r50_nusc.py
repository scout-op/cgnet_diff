...
# Load pretrained weights (only model, not optimizer)
# load_from = '/mnt/tf-mdriver-jfs/exps/lixiangjie/roadnet/data_copy/cgnet_diff/work_dir/epoch_15.pth'
...
model = dict(
    ...
    pts_bbox_head=dict(
        ...
        bev_h=bev_h_,
        bev_w=bev_w_,
        # 手动控制阶段：True=阶段2（JAQ+Topology）, False=阶段1（只有Geometry）
        force_stage2=False,
        # Topology endpoint distance threshold in meters.
        # Use 'auto' to derive from BEV resolution (recommended baseline).
        topology_dist_threshold='auto',
        transformer=dict(
        ...
        use_jaq=True,  # 暂时禁用 JAQ，先测试 topology
        use_bsc=False,
        use_gnn=True
    )
)
...
# 自定义 Hook：传递 epoch 给模型
custom_hooks = [
    dict(type='InjectEpochHook', priority='NORMAL')  # 注入 epoch 到 img_metas
]

work_dir = '/mnt/tf-mdriver-jfs/exps/lixiangjie/roadnet/data_copy/cgnet_diff/work_dir/t3'
