"""
FlowCGNet配置文件 (points模式 - 对齐原始CGNet)
Flow Matching 直接生成固定采样点序列 (P=num_pts_per_line)

说明:
- 本配置用于对比 control points 方案 (见 flow_cgnet_r50_nusc_complete.py / flow_cgnet_r50_nusc.py)
- points 模式下，FlowCenterlineHead 会直接生成固定数量的坐标点序列；
  但 x0_prior 仍由少量控制点预测并插值得到（实现上更稳定、也更省参数）。
"""
...
# ===== 训练参数 (可手动调整) =====
batch_size = 10       # 每个GPU的batch size
...
model = dict(
    ...
    pts_bbox_head=dict(
        ...
        flow_time_beta_a=0.5,
        flow_time_beta_b=2.0,
        flow_time_weighting='exp',
        flow_time_weight_lambda=2.0,
        # ===== loss 坐标缩放：按 pc_range 对齐 x/y 的米制尺度（归一化后保持量级稳定） =====
        flow_coord_scaling='pc_range',
        # ===== x0_prior：粗曲线 + per-point residual =====
        x0_prior_type='bezier_residual',
        x0_residual_scale=0.02,
        coord_min=0.01,
        coord_max=0.99,
        ...
        num_flow_layers=4,
        num_gnn_layers=6,
        num_sampling_steps=5,  # points模式建议>1步，单步更难
        pc_range=point_cloud_range,
        bev_h=bev_h_,
        bev_w=bev_w_,
        use_grpo=True,   # 保持与complete配置一致（可自行关闭）
        grpo_samples=4,
        grpo_start_epoch=12,
...
resume_from = '/mnt/tf-mdriver-jfs/exps/lixiangjie/roadnet/data_copy/cg/fg/work_dirs/flow_cgnet_r50_nusc_complete_v5/epoch_4.pth'


# resume_from = None
work_dir = '/mnt/tf-mdriver-jfs/exps/lixiangjie/roadnet/data_copy/cg/fg/work_dirs/flow_cgnet_r50_nusc_complete_v5'
