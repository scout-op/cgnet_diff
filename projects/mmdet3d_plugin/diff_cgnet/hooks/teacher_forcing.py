...
class ProgressiveTrainingScheduler:
    ...
    def __init__(self,
                 stage1_epochs=9,  # 测试：让 Epoch [3] 进入阶段 2
                 stage2_epochs=20,
...
class TeacherForcingModule(nn.Module):
    ...
    def forward(self, pred_ctrl, gt_ctrl, tf_prob, training=True):
        ...
        # Ensure gt_ctrl has same dtype as pred_ctrl
        gt_ctrl = gt_ctrl.to(pred_ctrl.dtype)
        ...
        # Convert mask to same dtype for FP16 compatibility
        use_gt_mask_expanded = use_gt_mask.unsqueeze(-1).unsqueeze(-1).to(pred_ctrl.dtype)
        gnn_input = use_gt_mask_expanded * noisy_gt + (1 - use_gt_mask_expanded) * pred_ctrl
