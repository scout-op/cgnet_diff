...
class BezierSpaceConnection(nn.Module):
    ...
    def compute_bezier_projection_matrix(self, input_dim, output_dim):
        ...
        # input_dim is the number of input points (2n), not the flattened
    ...
    def forward(self, lane_embeddings, lane_ctrl_points, connectivity):
        ...
        for b, i, j in connected_pairs:
            ...
            ctrl_i = lane_ctrl_points[b, i]  # (P, 2)
            ctrl_j = lane_ctrl_points[b, j]  # (P, 2)
            concat_points = torch.cat([ctrl_i, ctrl_j], dim=0)  # (2P, 2)

            # Project concatenated points to combined Bezier control points.
            bezier_proj = torch.matmul(self.bezier_matrix.to(device), concat_points)  # (C, 2)
            bezier_proj = bezier_proj.flatten()
