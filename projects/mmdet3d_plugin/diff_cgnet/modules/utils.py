...
def resample_polyline(points, num_points=20):
    ...
    Args:
        points: np.ndarray / torch.Tensor / shapely LineString-like, shape (N, 2)
        num_points: int, number of points to sample

    Returns:
        sampled_points: np.ndarray, shape (num_points, 2)
    """
    if hasattr(points, 'coords'):
        points = np.array(points.coords)
    ...
    if points.ndim != 2 or points.shape[1] < 2:
        raise ValueError(f'Invalid polyline shape: {points.shape}')
    ...
    cumdist = np.concatenate([[0.0], np.cumsum(seg_lens, dtype=np.float32)], axis=0)
    total = float(cumdist[-1])

    if total < 1e-6:
        return np.repeat(points[:1], num_points, axis=0)

    target = np.linspace(0.0, total, num_points, dtype=np.float32)
    xs = np.interp(target, cumdist, points[:, 0]).astype(np.float32)
    ys = np.interp(target, cumdist, points[:, 1]).astype(np.float32)
