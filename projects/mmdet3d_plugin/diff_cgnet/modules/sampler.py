...
from .utils import bezier_interpolate
...
class BezierDeformableAttention(nn.Module):
    ...
    def forward(self, 
        ...
        Args:
            ...
            ctrl_points: torch.Tensor, shape (B, N, 4, 2), 控制点（归一化到[0,1]，可能有噪声）
    ...
    def generate_reference_points(self, ctrl_points, pc_range):
        ...
        Returns:
            reference_points: torch.Tensor, shape (B, N, K, 2), BEV归一化坐标[0,1]
        """
        ...
        # ctrl_points are already normalized to BEV [0,1]; do NOT normalize again.
        reference_points = torch.clamp(dense_points, 0.01, 0.99)
