...
class AdvancedTopologyGNN(BaseModule):
    ...
    def __init__(self,
        ...
        # CGNet 风格 TopologyHead：两阶段边预测
        # 1. 节点嵌入生成器: MLP(D, D/2, D/4, 2层)
        self.association_embed_maker = nn.Sequential(
            nn.Linear(embed_dims, embed_dims // 2),
            nn.ReLU(inplace=True),
            nn.Linear(embed_dims // 2, embed_dims // 4)
        )
        # 2. 边分类器: MLP(D/2, D/4, 1, 2层)
        self.association_classifier = nn.Sequential(
            nn.Linear(embed_dims // 2, embed_dims // 4),
            nn.ReLU(inplace=True),
    ...
    def predict_edges(self, node_features):
        ...
        # 1. 生成节点嵌入 + sigmoid（CGNet 风格）
        vertex_embed = torch.sigmoid(self.association_embed_maker(node_features))  # [B, N, D/4]
        
        # 2. 两两配对
        vertex_features1 = vertex_embed.unsqueeze(2).expand(B, N, N, -1)  # [B, N, N, D/4]
        vertex_features2 = vertex_embed.unsqueeze(1).expand(B, N, N, -1)  # [B, N, N, D/4]
        vertex_features = torch.cat([vertex_features1, vertex_features2], dim=-1)  # [B, N, N, D/2]
        
        # 3. 边分类
        adj_logits = self.association_classifier(vertex_features).squeeze(-1)  # [B, N, N]
