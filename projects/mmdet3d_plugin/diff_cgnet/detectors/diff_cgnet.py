...
class DiffCGNet(MVXTwoStageDetector):
    ...
    def forward_train(self,
        ...
        # Expected training shape is (B, T, Ncam, C, H, W). Keep compatibility
        # with non-temporal (B, Ncam, C, H, W) inputs as well.
        if img is not None and img.dim() == 6:
            ...
            # Use meta of the latest frame for current forward.
            img_metas = [each[len_queue - 1] for each in img_metas]
        elif img is not None and img.dim() == 5:
            # No temporal queue; ensure img_metas is a list[dict] (not list[list[dict]]).
            if isinstance(img_metas[0], list):
                img_metas = [each[-1] for each in img_metas]
        elif img is not None and img.dim() == 4:
            # Single image tensor; keep as-is.
            pass
        else:
            raise ValueError(f"Unexpected img shape: {None if img is None else tuple(img.shape)}")
    ...
    def simple_test_pts(self, x, img_metas, prev_bev=None, rescale=False):
        ...
        # 直接使用 post_process 的输出，已经包含评估代码需要的所有字段
        bbox_results = bbox_list
