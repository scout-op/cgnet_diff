...
class LiDARInstanceLines(object):
    ...
    def __init__(self, 
        ...
        self.nums_control_pts = nums_control_pts
    ...
    def get_graph_gt(self):
        ...
        for instance in self.instance_list:
            ...
            sampled_points = np.array(instance.coords)
            line = to_tensor(sampled_points).to(dtype=torch.float32)
            line[:,0] = torch.clamp(line[:,0], min=-self.max_x,max=self.max_x)
            line[:,1] = torch.clamp(line[:,1], min=-self.max_y,max=self.max_y)
            instance_points_list.append(line)

        for i in range(len(instance_points_list)):
            connection = np.where(self.adj_matrix[i]==1)[0]
            for idx in connection:
                if torch.all(instance_points_list[i][-1]!= instance_points_list[idx][0]):
                    if torch.all(instance_points_list[i][0] == instance_points_list[idx][0]):
                        instance_points_list[i] = torch.flip(instance_points_list[i], dims=[0])
                    else:
                        instance_points_list[idx] = torch.flip(instance_points_list[idx], dims=[0])
        ...
        line_length = [len(line) for line in instance_points_list]
        ...
        if remove_list:
            ...
            instance_points = np.delete(instance_points.numpy(), remove_list, axis=0)

        G = nx.DiGraph(adj_matrix)

        for i, node in enumerate(G.nodes()):
            G.nodes[node]['pos'] = instance_points[i]

        return G

    @property
    def get_graph_gt_vis(self):
        ...
        for instance in self.instance_list:
            instance = instance.simplify(0.2, preserve_topology=True)
            sampled_points = np.array(instance.coords)
            line = to_tensor(sampled_points).to(dtype=torch.float32)
            line[:,0] = torch.clamp(line[:,0], min=-self.max_x,max=self.max_x)
            line[:,1] = torch.clamp(line[:,1], min=-self.max_y,max=self.max_y)
    ...
    @property
    def get_beizer_control_pts(self):
        """
        return torch.Tensor([N,fixed_num,2]), in xmin, ymin, xmax, ymax form
            N means the num of instances
        """
        ...
        instance_points_tensor[:,:,0] = torch.clamp(instance_points_tensor[:,:,0], min=-self.max_x,max=self.max_x)
        instance_points_tensor[:,:,1] = torch.clamp(instance_points_tensor[:,:,1], min=-self.max_y,max=self.max_y)
        B = self.get_bernstein_basis(self.fixed_num * 2, self.nums_control_pts)
        nums_line = len(self.adj_matrix)
        control_pts_matrix = torch.zeros((nums_line, nums_line, self.nums_control_pts, 2))
        inv_B = torch.linalg.pinv(B)
        for i in range(nums_line):
            connection = np.where(self.adj_matrix[i]==1)[0]
            for c in connection:
                new_line = torch.cat((instance_points_tensor[i], instance_points_tensor[c]))
                control_pts = torch.matmul(inv_B, new_line)
                control_pts_matrix[i, c] = control_pts
        return control_pts_matrix

    def get_bernstein_basis(self, nums_pts, nums_ctp):
        def comb(n, k):
    ...
    def key_points(self):
        ...
        instance_se_points_list = []
        for instance in self.instance_list:
            instance_se_points_list.append(instance.coords[0])
            instance_se_points_list.append(instance.coords[-1])
        return instance_se_points_list
...
class VectorizedLocalMap(object):
    ...
    def __init__(self,
                 ...
                 centerline_classes = ['lane', 'lane_connector'],
                 sample_dist=1,
                 num_samples=250,
                 padding=False,
                 fixed_ptsnum_per_line=-1,
                 padding_value=-10000,
                 only_centerline=False,
                 nums_control_pts=6):
        '''
        Args:
    ...
    def gen_vectorized_samples(self, location, lidar2global_translation, lidar2global_rotation):
        ...
        for vec_class in self.vec_classes:
            if vec_class == 'divider':
                ...
                line_geom, valid_records = self.get_map_centerline_geom(patch_box, patch_angle, self.centerline_classes, location)
                line_instances_dict = self.line_geoms_to_instances(line_geom)   
                adj_matrix = self.get_centerline_adj_matrix(line_instances_dict, location, valid_records)
                for line_type, instances in line_instances_dict.items():
                    for instance in instances:
                        vectors.append((instance, self.CLASS2LABEL.get('centerline', -1)))
            else:
        ...
        gt_instance = LiDARInstanceLines(gt_instance,self.sample_dist,
                        ...
                        adj_matrix=adj_matrix,nums_control_pts=self.nums_control_pts)
    ...
    def get_map_centerline_geom(self, patch_box, patch_angle, layer_names, location):
        map_geom = []
        map_geoms = []
        valid_records = []
        for layer_name in layer_names:
            if layer_name in self.centerline_classes:
                geoms, valid_record = self.get_center_line(patch_box, patch_angle, layer_name, location)
                map_geom.extend(geoms)
                valid_records.extend(valid_record)
        map_geoms.append(('centerline', map_geom))

        return map_geoms, valid_records

    def get_centerline_adj_matrix(self, centerlines, location, valid_records):
        centerlines = [np.array(line.coords) for line in centerlines['centerline']]  
        nums_centerline = len(valid_records)
        adj_matrix = np.zeros((nums_centerline, nums_centerline))
        for i in range(nums_centerline):
            token = valid_records[i]['token']
            outgoing_tokens = self.nusc_maps[location].get_outgoing_lane_ids(token)
            incoming_tokens = self.nusc_maps[location].get_incoming_lane_ids(token)
            for j in range(i+1, nums_centerline):
                if valid_records[j]['token'] in outgoing_tokens and (centerlines[i][-1]-centerlines[j][0]).sum()<0.01:
                    adj_matrix[i][j] = 1
                if valid_records[j]['token'] in incoming_tokens and (centerlines[i][0]-centerlines[j][-1]).sum()<0.01:
                    adj_matrix[j][i] = 1
    ...
    def get_center_line(self,patch_box,patch_angle,layer_name,location):
        ...
        for record in records:
            ...
            if polygon.is_valid:
                ...
                new_polygon = polygon.intersection(patch)
                if not new_polygon.is_empty:
                    centerline = self.map_explorer[location].map_api.discretize_lanes(
                            record, 0.5)
                    centerline = list(self.map_explorer[location].map_api.discretize_lanes([record['token']], 0.5).values())[0]
                    centerline = LineString(np.array(centerline)[:,:2])
                    centerline = centerline.intersection(patch)
                    centerline = self.to_patch_coord(centerline, patch_angle, patch_x, patch_y)
                    if not centerline.is_empty:
                        if centerline.geom_type == 'MultiLineString':
                            for l in centerline.geoms:
                                centerline_list.append(l)
                                valid_records.append(record)
                        else:
                            centerline_list.append(centerline)
                            valid_records.append(record)

        return centerline_list, valid_records

    def to_patch_coord(self, new_polygon, patch_angle, patch_x, patch_y):
        new_polygon = affinity.rotate(new_polygon, -patch_angle,
                                    origin=(patch_x, patch_y), use_radians=False)
        new_polygon = affinity.affine_transform(new_polygon,
                                                [1.0, 0.0, 0.0, 1.0, -patch_x, -patch_y])
...
class CustomNuScenesLocalMapDataset(CustomNuScenesDataset):
    ...
    def __init__(self,
                 ...
                 only_centerline=False,
                 graph_ann_file=None,
                 openlane_ann_file=None,
                 nums_control_pts=6,
                 *args, 
                 **kwargs):
        super().__init__(*args, **kwargs)
        self.map_ann_file = map_ann_file
        if graph_ann_file == None and map_ann_file != None:
            path = self.map_ann_file.split('/')
            path[-1] = "graph_" + self.map_ann_file.split('/')[-1][:-4] + "pkl"
            graph_ann_file = "/".join(path)
        if openlane_ann_file == None and map_ann_file != None:
            path = self.map_ann_file.split('/')
            path[-1] = "openlane_" + self.map_ann_file.split('/')[-1][:-4] + "pkl"
            openlane_ann_file = "/".join(path) 
        self.graph_ann_file = graph_ann_file
        self.openlane_ann_file = openlane_ann_file
        ...
        self.vector_map = VectorizedLocalMap(kwargs['data_root'], 
                            ...
                            padding_value=self.padding_value, only_centerline=only_centerline,
    ...
    def get_data_info(self, index):
        ...
        if self.modality['use_camera']:
            ...
            for cam_type, cam_info in info['cams'].items():
                ...
                ego2img = camera_intrinsics @ np.linalg.inv(camera2ego)
        ...
        if not self.test_mode:
            annos, _, _ = self.get_ann_info(index)
    ...
    def _format_gt(self):
        ...
        if (not os.path.exists(self.map_ann_file)) or (not os.path.exists(self.graph_ann_file)) \
            or (not os.path.exists(self.openlane_ann_file)):
            ...
            for sample_id in range(dataset_length):
                ...
                for i, (gt_label, gt_vec) in enumerate(zip(gt_labels, gt_vecs)):
                    ...
                    openlane_anno = dict(
                        id=i,
                        points=np.array(list(gt_vec.coords)),
                        confidence=1)
                    gt_openlane_vec_list.append(openlane_anno)

                gt_anno['vectors']=gt_vec_list
                gt_annos.append(gt_anno)
                gt_graph = gt_sample_dict['gt_bboxes_3d'].data.get_graph_gt
                gt_graph_annos.append(gt_graph)

                #openlane format
                openlane_gts[sample_token] = {
                    'annotation': {
                        'lane_centerline':gt_openlane_vec_list,
                        'topology_lclc':gt_sample_dict['gt_bboxes_3d'].data.adj_matrix
                    }
                }
            ...
            print('\n GT graph anns writes to', self.graph_ann_file)
            with open(self.graph_ann_file, 'wb') as f:
                pickle.dump(gt_graph_annos, f)
            print('\n GT anns writes to', self.map_ann_file)
            mmcv.dump(nusc_submissions, self.map_ann_file)
            print('\n GT openlane anns writes to', self.openlane_ann_file)
            mmcv.dump(openlane_gts, self.openlane_ann_file)
        else:
    ...
    def _format_bbox(self, results, jsonfile_prefix=None):
        ...
        for sample_id, det in enumerate(mmcv.track_iter_progress(results)):
            ...
            for i, vec in enumerate(vecs):
                ...
                openlane_anno = dict(
                    id=i,
                    points=vec['line'],
                    confidence=vec['score'])
                openlane_vec_list.append(openlane_anno)

            pred_anno['vectors'] = pred_vec_list
            pred_annos.append(pred_anno)
            if 'graph' in det:
                pred_graph_annos.append(det['graph'])

            #openlane format
            openlane_results['results'][sample_token] = {
                'predictions': {
                    'lane_centerline':openlane_vec_list,
                    'topology_lclc':det['adj_matrix']
                }
            }
        ...
        if not os.path.exists(self.map_ann_file) or not os.path.exists(self.graph_ann_file) \
            ...
            print(f'{self.map_ann_file} and {self.graph_ann_file} and {self.openlane_ann_file} exist, not update')
        ...
        res_graph_path = osp.join(jsonfile_prefix, 'nuscgraph_results.pkl')
        with open(res_graph_path, 'wb') as f:
            pickle.dump(pred_graph_annos, f)
        print('Graph results writes to', res_graph_path)

        res_openlane_path = osp.join(jsonfile_prefix, 'nuscopenlanemap_results.pkl')
        with open(res_openlane_path, 'wb') as f:
            pickle.dump(openlane_results, f)
        print('Openlane results writes to', res_openlane_path)
        return res_path, res_graph_path, res_openlane_path
    ...
    def _evaluate_single(self,
        ...
        from projects.mmdet3d_plugin.datasets.openlane.opnelane_eval import openlane_evaluate

        map_result_path = result_path[0]
        graph_result_path = result_path[1]
        openlane_result_path = result_path[2]

        detail = dict()
        
        if "chamfer" in metric:
            result_path = osp.abspath(map_result_path)      
            print('------Evaluate by chamfer------')
            ...
            for i, name in enumerate(self.MAPCLASSES):
                ...
                detail['NuscMap/{}_AP'.format(name)] =  cls_aps.mean(0)[i]
            print('map: {}'.format(cls_aps.mean(0).mean()))
            detail['NuscMap/mAP'] = cls_aps.mean(0).mean()

            for i, name in enumerate(self.MAPCLASSES):
                for j, thr in enumerate(thresholds):
                    detail['NuscMap/{}_AP_thr_{}'.format(name,thr)]=cls_aps[j][i]

        if "topology" in metric:
            print('------Evaluate by topology------')
            with open(graph_result_path,'rb') as f:
                gen_results = pickle.load(f)
            with open(self.graph_ann_file,'rb') as ann_f:
                annotations = pickle.load(ann_f)

            metric_names = [
                "TOPO Precision", "TOPO Recall", "TOPO F1", "JTOPO F1",
                "GEO Precision", "GEO Recall", "GEO F1",
                "APLS", "Graph IoU", "SDA"]


            evaluator = GraphEvaluator(radius=5, interp_dist=2.5, prop_dist=80, gmode='direct')
            metrics_all = []
            split = len(annotations)//nProc + 1
            for i in tqdm(range(split)):
                with Pool(nProc) as pool:
                    metrics_all.extend(pool.starmap(
                            partial(evaluator.evaluate_graph), zip(annotations[nProc*i: nProc*(i+1)], gen_results[nProc*i: nProc*(i+1)])))

            for metric_name in metric_names:
                detail[metric_name] = np.nanmean(
                    [metric[metric_name] for metric in metrics_all])

        if "openlane" in metric:
            # eval openlane topo  
            print('------Evaluate by openlane------')
            with open(openlane_result_path,'rb') as f:
                gen_results = pickle.load(f)
            with open(self.openlane_ann_file,'rb') as ann_f:
                annotations = pickle.load(ann_f)

            results = openlane_evaluate(
    ...
    def evaluate(self,
        ...
        # 调用 _format_bbox 格式化结果，返回 (map_path, graph_path, openlane_path)
        result_files = self._format_bbox(results, jsonfile_prefix)
        
        # _evaluate_single 期望 tuple of 3 paths
    ...
    def get_ann_info(self, index):
        ...
        try:
            gt_ins_id = np.array(info['instance_inds'])[mask]
        except:
            gt_ins_id = None
        gt_labels_3d = []
        ...
        return anns_results, gt_bboxes_3d, gt_ins_id


def output_to_vecs_graph(detection):
    if isinstance(detection['line'], list):
        line = np.array([pts.cpu().numpy() for pts in detection['line']])
    else:
        line = detection['line'].numpy()
    line_scores = detection['line_scores'].numpy()
    line_labels = detection['line_labels'].numpy() 

    vec_list = []
    for i in range(line_labels.shape[0]):
        vec = dict(
            label=line_labels[i],
            score=line_scores[i],
            line=line[i],
