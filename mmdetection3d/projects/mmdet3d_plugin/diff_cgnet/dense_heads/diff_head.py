...
class DiffusionCenterlineHead(nn.Module):
    ...
    def forward_single_step(self, noisy_ctrl, bev_features, t, self_cond=None):
        ...
        # Ensure bev_features is 4D (B, C, H, W)
        if bev_features.dim() == 5:
            # (B, N, C, H, W) -> take first view or merge
            bev_features = bev_features[:, 0, :, :, :]
        elif bev_features.dim() == 3:
            # (B, C, HW) -> (B, C, H, W)
            B, C, HW = bev_features.shape
            H = self.bev_h
            W = self.bev_w
            bev_features = bev_features.reshape(B, C, H, W)
    ...
    def loss(self, pred_ctrl, pred_logits, features,
        ...
        # 对于单类别任务，pred_logits 是 (B, N, 1)，使用 BCE Loss
